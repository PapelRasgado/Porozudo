...
def register_rpg_commands(bot: Bot):
    ...
    async def rpg(
        ctx: ApplicationContext, question: Option(str, "Pergunta sobre Arkham RPG", name="pergunta", required=True)
    ):
    ...
    async def teste(
        ...
        try:
            ...
            docs = db.similarity_search_with_score(question, k=5, filter=search_filter)  # Filtro corrigido aqui
